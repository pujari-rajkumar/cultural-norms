# different speaker prediction Bert
...
def collate_fn(data):
    ...
    return x_batch, targets, (dialog_id_batch, utter_id_batch)
...
def run():
    ...
    train_dataset = MPDD(train_dialogs, context=False, sep_token=sep_token)

    val_dialogs = {k : dialogs[k] for k in val_dialog_keys}
    val_dataset = MPDD(val_dialogs, context=False, sep_token=sep_token)

    test_dialogs = {k : dialogs[k] for k in test_dialog_keys}
    test_dataset = MPDD(test_dialogs, context=False, sep_token=sep_token)
